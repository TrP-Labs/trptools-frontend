<script lang="ts">
	...
	interface Props {
		...
		/** The current page, shown after the group and not linked. */
		current: string;
	}

	let { group, current }: Props = $props();
</script>

<nav aria-label={m.layout_group_crumb_breadcrumb()} class="flex items-center gap-2 text-sm">
	<a
		href="/g/{group.slug}"
